...
public final class StreamingServiceExtractor implements Extractor {
    ...
    public void extract(ParseContext ctx) {
        ...
        for (var e : section.entrySet()) {
            ...
            flatten(e.getValue()).forEach(pat -> emit(ctx, input, value, pat));
        }
    }
    ...
    private static void emitStringOrRegex(ParseContext ctx, String input, String value, String s) {
        if (s.startsWith("re:")) {
            emitRegex(ctx, input, value, s.substring(3));
        } else {
            emitString(ctx, input, value, s, true);
        }
    }
    ...
    private static void emitStringOrList(ParseContext ctx, String input, String value, Object v, boolean ignoreCase) {
        if (v instanceof String str) {
            emitString(ctx, input, value, str, ignoreCase);
        } else if (v instanceof List<?> l) {
            l.forEach(x -> emitString(ctx, input, value, x.toString(), ignoreCase));
        }
    }

    private static void emitRegexOrList(ParseContext ctx, String input, String value, Object v) {
        if (v instanceof String str) {
            emitRegex(ctx, input, value, str);
        } else if (v instanceof List<?> l) {
            l.forEach(x -> emitRegex(ctx, input, value, x.toString()));
        }
    }

    private static void emitString(ParseContext ctx, String input, String value, String needle, boolean ignoreCase) {
    ...
    private static void emitRegex(ParseContext ctx, String input, String value, String src) {
        ...
        p.matcher(input).results().forEach(matchResult -> {
    ...
    private static boolean boundsOk(ParseContext ctx, String input, int s, int e) {
        boolean leftOk = s == 0 || Seps.isSep(input.charAt(s - 1)) || abutsStreamingTagged(ctx, s, true);
        boolean rightOk = e >= input.length() || Seps.isSep(input.charAt(e)) || abutsStreamingTagged(ctx, e, false);
    ...
    public void postProcess(ParseContext ctx) {
        var input = ctx.input;
        ctx.matches.named(MatchName.STREAMING_SERVICE)
                .filter(s -> !hasSuffixNeighbor(ctx, input, s) && !hasPrefixNeighbor(ctx, input, s))
                .toList()
    ...
    private static boolean hasSuffixNeighbor(ParseContext ctx, String input, Match s) {
        ...
        return atPosHasSuffix && cleanGap && (sStart == 0 || Seps.isSep(input.charAt(sStart - 1)));
    }
    ...
    private static boolean hasPrefixNeighbor(ParseContext ctx, String input, Match s) {
        ...
        return atPosHasPrefix && cleanGap && (sEnd >= input.length() || Seps.isSep(input.charAt(sEnd)));
    }

    private static boolean betweenIsSeps(String input, int from, int to) {
        if (from > to) return false;
        return input.substring(from, to).chars().allMatch(c -> Seps.isSep((char) c));
    }
}